...
import { SfcDefaultError } from "../models/errors/sfc-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | SfcDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | SfcDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new SfcDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new SfcDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
